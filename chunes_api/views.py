...
def tune_create(request):
  ...
  if request.user.is_authenticated:
    if request.method == 'POST':
      ...
      return JsonResponse({'message': 'no post'})
  else:
...
def tune_edit(request, pk):
  ...
  if request.user.is_authenticated:
    if request.method == 'POST':
      ...
      print(tune_list)
      # title = tune_list[0][3:]
      time_signature = tune_list[0][3:]
      default_note_length = tune_list[1][3:]
      tune_type = tune_list[2][3:]
      key_signature = tune_list[3][3:]
      # melody_line = '\n'.join(tune_list[5:])
      poster = Profile.objects.get(user=request.user)
      is_public = True
      tune = Tune.objects.filter(pk=pk).update(time_signature=time_signature, default_note_length=default_note_length, tune_type=tune_type, key_signature=key_signature, melody_line=melody_line, poster=poster, is_public=is_public)
...
def tune_delete(request, pk):
    ...
    return JsonResponse({'message': 'gone...'})
...
def post_show(request, pk):
  tune = Tune.objects.get(pk=pk)
  post = Post.objects.filter(tune=tune)
  print('showing ', tune.poster.user.username)
  chune = Tune.objects.filter(pk=pk).values().first().copy()
  print('choosing ', chune)
  print('mel', chune['melody_line'])
  notato = []
  ...
  post['poster'] = tune.poster.user.username
  print('posty', post)
  return JsonResponse({"post": post, "chune": chune})
